...
import { contains, getBoundingBox, getPortConstraints, reversePortConstraints, } from '../../util/mathUtils';
import Point from '../geometry/Point';
import CellState from '../cell/CellState';
import { DEFAULT_MARKERSIZE, DIRECTION, DIRECTION_MASK, ELBOW, ENTITY_SEGMENT, NONE, } from '../../util/Constants';
...
class EdgeStyle {
    ...
    static scalePointArray(points, scale) {
        ...
        if (points != null) {
            for (let i = 0; i < points.length; i += 1) {
                if (points[i] != null) {
                    const pt = new Point(Math.round((points[i].x / scale) * 10) / 10, Math.round((points[i].y / scale) * 10) / 10);
                    result[i] = pt;
                }
                else {
                    result[i] = null;
